...
res <- lfcShrink(dds, contrast = contrast, type = "normal")  # alt. type="apeglm" other package
res_df <- res %>% data.frame() %>% arrange(padj)             # sort by adjusted p-value
```
...
sig_res <- res_df %>% rownames_to_column(var="gene") %>% filter(padj < padj.cutoff)
```

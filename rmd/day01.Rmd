...
dds <- DESeqDataSetFromMatrix(
  countData=data %>% column_to_rownames("gene_id"), # move gene_id to rowname
  colData=metadata, 
  design= my_design)
```
...
> **Tasks**: Inspect the data structures and dimensions of objects dds and extract the count matrix Y 
(Hint: > ?counts)

> **Tasks**: Extract the first 10 rows (genes) from count matrix Y and safe as new data object -> Yr <- (reduced size for further use)
...
( Yr <- Y %>% head(10) )    # just the first few for illustrations
#Yr <- Y %>% data.frame() %>% sample_n(10)           # random rows
```

## 2. Visualize

> **Tasks**: Create heatmap for the first view genes in Yr (?pheatmap). How would you add metadata as below?

```{r visual_explore}
pheatmap(Yr, cluster_rows=FALSE, cluster_cols=FALSE, main="First glimpse")

# get metadata from dds and use it for colour annotation of pheatmap
...
> **Tasks**: understand the object "meta" and add sequence depth as additional annotation for all samples (additional column)
...
**Notice**:  'meta=colData(dds)' is identical to 'metadata', but
- 'meta' has rownames (required by pheatmap)
...
```{r correlations}
C <- cor(Y)       # get sample-sample correlations: other correlations?
pheatmap(C, annotation = meta)
```
...
# pheatmap(C, annotation = meta, annotation_colors = my_colors, color=blue_colors) 
```
...
> **Task** Look at the quantiles and means of the first 10 genes (from Yr) and create a boxplot

...
In this study, RNA-seq data is available to compare amiloride to the state-of-the-art drug (TG003) in two cancer (multiple myeloma) cell lines with different mutations (BM: p53mut, JJ: del(17p)).
...
1. Start a new session in [workbench](https://workbench.ie-freiburg.mpg.de), or on your local computer.
...
In principle, you can download the published data for our tutorial from [GEO](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE95077)

For this course, we have already prepared (all!) the data in a convenient format.
...
Often the metadata is hidden in the paper (or in the filename). This is bad practise - make sure to have a proper sample sheet. Metadata is just as important as data.

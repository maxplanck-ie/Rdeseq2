...
summary(Y[,1:3])  # M %>% data.frame() %>%  select(1:3) %>% summary()
boxplot(Y[,1:3])  # M %>% data.frame() %>%  select(1:3) %>% boxplot()
```
...
Y %>% 
  ...
  ggplot(aes(x=sample, y=log_count, color=sample)) +
  geom_boxplot()
...
```{r explore_corr}
Yt <- log2(M + 1)      # simple log-transform; (why + 1 ?)

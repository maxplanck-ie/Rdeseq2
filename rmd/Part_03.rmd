...
Cave!
If we want to use shrinking, be careful with the apegml method. ashr-method is a bit more versatile.
...
We have two options now. Or we re-run deseq with releved factors, or we dive a bit in the contrast setting of results.
...
cleanDF <- function(RESobj){
  ...
  # Take care of counts
  RESdf <- RESdf[RESdf$baseMean != 0,]
  # Take care of Pval NA's
  RESdf[is.na(RESdf)] <- 1
  return(RESdf[order(RESdf$padj, decreasing = FALSE),])
}
...
pheatmap(
  counts(dds, normalized=TRUE)[head( rownames(TGvAMIL_JJ_cleaned) , n=20), ],
  scale = 'row',
  main='TG v AMIL - JJ'
  )

pheatmap(
  counts(dds, normalized=TRUE)[head( rownames(TGvAMIL_BM_cleaned) , n=20), ],
  scale = 'row',
  main='TG v AMIL - JJ'
  )
```

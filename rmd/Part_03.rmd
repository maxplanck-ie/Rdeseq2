...
# If you already have the dds object just replace the design by:
    #  dds <- DESeq(dds, design = design)
dds <- DESeq(dds)
```
...
DESeq2 also offers the Likelihood Ratio Test (LRT) as an alternative to wilde test when evaluating expression change across more than two levels. Rather than evaluating whether a gene’s expression is up- or down-regulated in one class compared to another, it identifies genes which are changing in expression in any direction across the different factor levels. Therefore, there is no need for contrasts since we are not making a pair-wise comparison.

...
2. Create a new project directory, e.g. DESeq2_Course

**Remember**: 
Many useful functions are already available with R: base packages.

Loading new packages enables new functionalities for a given R-session.
...
**Anynymous Poll**: Who has successfully installed the following packages: DESEeq2, tidyverse, pheatmap ?
...
Notice that this may result in multiple (sometimes redundant) functions with the same name: e.g. sd()
...
In principle, you can download the data from GEO but we have already prepared the data in a convenient format

Location: /data/processing/Rdeseq2/counts_qc.tsv

### Import Data 
Data comes in various formats. For now we assume that the data is available as tab-separated count matrix.
...
**Discussion** Inspect the data. What does R mean with variables? What are observations? 

**Poll**: How many samples are in the data
...
### Import Metadata
Metadata contains further important information on the samples and their origin. Typically these are categorical data ("factors") such as cell type, experimental condition or other batch assignments.
But it could also be quantitative data (such as age).

Often the metadata is hidden in the paper (or in the filename). This is bad practise - make sure to have a clean sample sheet. Metadata is just as important as data.

```{r import_meta}
mfile="../data/counts_qc_meta.tsv"                     # location of metadata file
metadata <- read.csv(file=mfile, header=TRUE, sep="\t")
metadata <- metadata %>% column_to_rownames("sample")  # geneID -> rownames
...
**Task**: Read the metadata into an object "metadata". Make sure that the columns "condition" and "celltype" are factors. 
...
**Goal**: model expected counts $y$ as a function of sample $x$ (c.f. linear regression)
...
**Poll**: What is $N$ in this example
...
- none: each sample have their own values
- quantitative variable: e.g. sequence depth
- categorical variable: e.g. condition
- multiple variable: e.g. condition and celltype
...
**Discussion**: How to convert sample condition into something numerical?
...
$$
y_i = \beta_0 + \beta_1 x_i \longrightarrow \beta_0 X_{0i} + \beta_1 X_{1i} + \beta_2 X_{2i} = X \cdot \beta
$$

(c.f. linear regression $\to$ multivariate linear regression)


**Upshot**: A design can be translated into a model matrix
```{r model_matrix}
MM=model.matrix(my_design, data=metadata)
pheatmap(MM, cluster_cols = FALSE, cluster_rows=FALSE)
```

**Tasks** (for Part II/III): 
...
### Interlude: Experimental Design (10min)
...
Batches, Replicates, Sequencing Depth


# Create DESeq2 object (10 min)
...
**Task 1**: Inspect the data structures and dimensions of objects dds and extract the count matrix M (Hint: counts())
...
# genome-wide correlation matrix --> notice high correlation (most genes unaffected)
...
Similar to correlation analysis the main purpose is to identify similar samples.
```{r PCA_manual}
nt <- 500
top <- order(rowVars(Mt), decreasing = TRUE)[1:nt] # top variable genes (excl. zero variable)
pca <- prcomp(t(Mt[top,]), scale=TRUE)
eigs <- pca$sdev^2
vars <- round(100*eigs / sum(eigs), 2)

rld_PCA = as.data.frame(pca$x[,1:2])                   # only take PC1 and PC2 for plotting

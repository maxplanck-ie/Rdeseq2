...
**Discussion** Inspect the data structure. What are the variables? What are observations? How many samples are in the data ?
...
> **Poll 5**: What is $n$ in our example
...
**Notice**: Some *nuisance factors* (e.g. batches) may be known and measurable, but are not of primary interest - they should be included in the model.
...
```{r design, echo=TRUE}
my_design <- ~ condition   # here: X = condition
```

**Notice**: The variables need to correspond to columns in the metadata.

> **Task**: Which other variables could be used to define the design?

**Notice**: More complicated designs are possible and often necessary ($\to$ day 2 and 3)
...
MM=model.matrix(~ 0 + condition*celltype, data=metadata)
pheatmap(MM, cluster_cols = FALSE, cluster_rows=FALSE)
```
...
- known and interesting: interaction effects and multi-factorial design (genotype, treatment)
...
> **Poll 5** You are planning an experiment with 3 treatment levels $(X_1, X_2, X_3)$ and aim for 3 replicates each. Unfortunately, you can process at most 3 samples per day. So you'll need three days $(Z_1, Z_2, Z_3)$. How should you assign your samples to different treatments?
...
|-------|:-----|
Treatment            | Observe!?
Biological           | Estimate through **replication**
Sample Preparation   | Use batch controls (**blocking**)
Unknown Confounders  | reduce risk by **randomization** 
Sequencing           | control sequencing depth (read sampling)

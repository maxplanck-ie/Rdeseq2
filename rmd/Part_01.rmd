...
Use RNA-seq to compare two drugs (amiloride and TG003) in two cancer cell lines with different mutations (BM: p53mut, JJ: del(17p))
...
**Recommendation**: Please use *Notebooks* throughout this course
...
**Practical Notice**: Some *nuisance factors* (e.g. batches) may be known and measurable, but are not of primary interest - they should be included in the model.

$$
y_i = \mu + \beta x_i + \gamma z_i + \epsilon_i \\ ~~\\
\mbox{R syntax:} ~~~Y \sim X + Y
$$
...
#cn_col <- RColorBrewer::brewer.pal(n=3, "Dark2")            # avoid RColorBrewer dependence for this course

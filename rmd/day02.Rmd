...
- filterig: samples and genes
- transformation: normalization and sizefactors
- modelling data: variance + dispersion
...
### Load Metadata
```{r load_meta_data, message=FALSE, echo = TRUE, results = 'hide'}
...
> **Poll 1:** Do you notice any issue from this PCA plot? What would you do about it - 5 min
...
```{r filter_samples, out.width = '60%', fig.align="center"}
dds_clean <- dds[, -c(1,7)] 
myPlotPCA(dds_clean)
```
...
- remove genes (rows) that have very low counts (very unlikely to have biological meaning)
- arbitrary threshold: < 1, < 10
- benefit: reduces the memory size that the 'dds' object occupies => increases the speed
  


**Task 2:** Filter out the genes that have a total amount of only zero or 1 counts.

Hint: Define first `dds_clean %>% counts %>% rowSums() %>% head()`
...
Internally they are stored as integers and per default they aree sorted *alphanumerically*. 
It may be more convenient to sort them by some user defined order - this is called releveling
...
> **Task** Reorder factors with 'DMSO' as baseline - 2 min.
...
#dds_clean$condition <- factor(dds_clean$condition, levels = c ('DMSO', 'TG', 'AMIL')) 
...
- define DESeq data object
  ...
  - design: `~condition`   -- which variable to be modeled. formula will be converted to design matrix with dummy variables
- Filtering & Transformation & Relevel

> This is **before** any statistical analysis !

> A real-world analysis would likely be iterative
  
<br>

<center>

![DESeq2 workflow](images/part00_workflowdeseq2.png){width=80%}
...
**Message**:
> Document your decisions, parameters, workflows and software version carefully - use notebooks !

...
meta$seq_depth <- counts(dds_clean) %>% colSums()/1e6 
...
# colSums(dds_clean) are not strictly the sequencing depth (after gene filtering), but
# colSums(dds) may contain more samples (before sample filtering)
sd = dds %>% counts %>% colSums %>% tibble(sample=names(.), seq_depth=./1e6)
# therefore we ne 
meta 

...
* Different steps of a differential gene expression analysis
  ...
  * Dispersion estimation (has been explained before)
...
```{r fulldesign}
dds <- DESeqDataSetFromMatrix(countData=data, colData=metadata, design= ~celltype+condition+celltype:condition)
dds <- DESeq(dds)
resultsNames(dds)
```
...
> **Task**: Plot a heatmap of the normalised counts for the 'top 20' genes from our previously generated results table.
  ...
  * Extract the normalised (!) counts from the dds object and convert to dataframe ( counts(dds) )

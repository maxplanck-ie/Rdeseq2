...
```{r plot_design_noint}
pheatmap(
  model.matrix(~ 0 + condition, data=metadata),
  cluster_cols = FALSE,
  cluster_rows=FALSE,
  main="~ 0 + condition"
)
...
tdf <- coeflist_to_ggplot(dds, "ENSG00000198932", int=FALSE)
tdf$name <- factor(tdf$name, levels=resultsNames(dds))
ggplot(data=tdf, aes(x=name, y=value)) +
  theme_minimal() + 
  ggtitle('ENSG00000198932') +
  ...
  geom_segment(
     arrow = arrow(),
     aes(xend = name, y=c(0,0), yend=value-0.4,  color='red')
  ) +
  geom_point(size=10) +
  theme(legend.position = "none") +
  geom_text(aes(x=name, y=value+1, label=c(resultsNames(dds)[0], resultsNames(dds)[1])))
```
...
dds <- DESeqDataSetFromMatrix(
  countData=data, colData=metadata, design= ~0+celltype+condition+celltype:condition
)
dds <- DESeq(dds)
plotDispEsts(dds)
```
...
tdf <- coeflist_to_ggplot(dds, "ENSG00000005102", int=FALSE)
tdf$name <- factor(tdf$name, levels=resultsNames(dds))
ggplot(data=tdf, aes(x=name, y=value)) +
  theme_minimal() + 
  ggtitle('ENSG00000005102') +
  ...
  geom_segment(
     arrow = arrow(),
     aes(xend = name, y=c(0,0,0,0,0,0), yend=value + c(-0.6, 0.6, 0.6, 0.6, -0.6, -0.6),  color='red')
  ) +
  geom_point(size=10) +
  theme(legend.position = "none") +
...
my_design_noint <- as.formula(~ 0 + celltype + condition + celltype:condition)
...
dds_noint <- DESeq2::DESeqDataSetFromMatrix(countData = data, colData = metadata, design = my_design_noint)
# filtering as in day 1 and 2
dds <- dds[, -c(1,7)]                   # sample filtering
dds <- dds[counts(dds) %>% rowSums() > 1, ]  # gene filtering
dds_noint <- dds_noint[, -c(1,7)]                   # sample filtering
dds_noint <- dds_noint[counts(dds_noint) %>% rowSums() > 1, ]  # gene filtering
```
...
res_dds <- results(dds)
res_dds <- res_dds %>% data.frame()
res_dds %>% head()
res_dds <- res_dds %>% drop_na
res_dds_noint <- results(dds) %>% data.frame() %>% drop_na()
```
...
How is this for our cell-means model ?
...
```{r comp mods}
ggplot() +
  geom_point(aes(x=res_dds$log2FoldChange, y=res_dds_noint$log2FoldChange)) +
  theme_minimal() +
  xlab("Log2FC - intercept") +
  ylab("Log2FC - no-intercept")
```
...
res_noint <- results(dds_noint, contrast = contrast, alpha = 0.05)
res <- res %>% data.frame() %>% drop_na()
res_noint <- res_noint %>% data.frame() %>% drop_na()
res %>% head()
```
...
```{r comp mods amil_dmso}
ggplot() +
  geom_point(aes(x=res$log2FoldChange, y=res_noint$log2FoldChange)) +
  theme_minimal() +
  xlab("Log2FC - intercept") +
  ylab("Log2FC - no-intercept") +
  ggtitle(" c(\"condition\", \"AMIL\", \"DMSO\") ")
```


```{r contrast2, message=FALSE, warning=FALSE}
res2 <- results(dds, contrast = c("condition", "AMIL", "TG"), alpha = 0.05)
res2_noint <- results(dds_noint, contrast = c("condition", "AMIL", "TG"), alpha = 0.05)
res2 <- res2 %>% data.frame() %>% drop_na()
res2_noint <- res2_noint %>% data.frame() %>% drop_na()
ggplot() +
  geom_point(aes(x=res2$log2FoldChange, y=res2_noint$log2FoldChange)) +
  theme_minimal() +
  xlab("Log2FC - intercept") +
  ylab("Log2FC - no-intercept") +
  ggtitle(" c(\"condition\", \"AMIL\", \"TG\") ")
```
...
res2 <- res2 %>% data.frame %>% drop_na()
res2_noint <- results(dds_noint, contrast = list(c("conditionDMSO", "celltypeJJ.conditionDMSO")), alpha = 0.05)
res2_noint <- res2_noint %>% data.frame() %>% drop_na()
ggplot() +
  geom_point(aes(x=res2$log2FoldChange, y=res2_noint$log2FoldChange)) +
  theme_minimal() +
  xlab("Log2FC - intercept") +
  ylab("Log2FC - no-intercept") +
  ggtitle(" list(c(\"condition_DMSO_vs_AMIL\", \"celltypeJJ.conditionDMSO\")) //  ")
```
...
print("No Intercept:")
print(resultsNames(dds_noint))
```

```{r}
res2 <- results(dds, contrast = c(0,1,0,0,0,0), alpha = 0.05)
res2 <- res2 %>% data.frame %>% drop_na()
res2_noint <- results(dds_noint, contrast = c(-1,1,0,0,0,0), alpha = 0.05)
res2_noint <- res2_noint %>% data.frame() %>% drop_na()
ggplot() +
  geom_point(aes(x=res2$log2FoldChange, y=res2_noint$log2FoldChange)) +
  theme_minimal() +
  xlab("Log2FC - intercept") +
  ylab("Log2FC - no-intercept") +
  ggtitle(" c(0,1,0,0,0,0) <-> c(-1,1,0,0,0,0) ")
```

Let's make some exploratory plots.

```{r plot norm_count vs p values, warning=FALSE}
ggplot(data=res) +
...
```{r}
ggplot(data=res) +
  geom_point(size=0.5, aes(x=log10(baseMean+1), y=log2FoldChange)) +
  theme_minimal()
```
...
### a. Visualisation
  Several ways to visualize the DESeq2 outputs:
...
```
  * Volcano plot / MAplot
  ...
  Commonly used plots to have a global view of the expression are:
...
ggplot(data=res_shrink %>% data.frame(), aes(y=log2FoldChange, x=log10(baseMean))) +
  geom_point(size=0.5, color=ifelse(data.frame(res_shrink)$padj < 0.05, '#d62728', '#7f7f7f')) +
  theme_minimal()
...
```
> ! Cave ! The 'colMeans trick' needs BALANCED designs. An example where this goes wrong:

```{r}
fake_coldata <- data.frame(
    cond = c("DMSO", "DMSO", "DRUG", "DRUG", "DRUG", "DMSO", "DMSO", "DRUG", "DRUG"),
    genotype = factor(c("WT", "WT", "WT", "WT", "WT", "KO", "KO", "KO", "KO"), levels=c("WT", "KO"))
)
fake_modmat <- model.matrix(~cond * genotype, fake_coldata)

a <- fake_modmat[fake_coldata$cond == "DMSO",]
b <- fake_modmat[fake_coldata$cond == "DRUG",]
colMeans(b) - colMeans(a)

```
This is of course wrong. Note what happens under a balanced design:

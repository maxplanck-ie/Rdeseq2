...
dds <- dds[, -c(column_number_sample1, column_number_sample2)]
...
plot(log2(counts(dds)[,c(3,10)]))
plot(assay(rld)[,c(3,10)])               # 'assay()' allows you to extract the counts from the 'rld' object
```
...
df_pca_data=data.frame(PC1 = th.pca$x[,1], PC2 = th.pca$x[,2], sample = colData(dds)$celltype, condition=colData(dds)$celltype)  #! !Extract the needed info from dds with colData instead of using the 'metadata' ('metadata' needs to be corrected too if you drop samples!)

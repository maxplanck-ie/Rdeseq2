...
df <- bind_rows(
  as_tibble(log2(counts(fdds)[, 1:2]+1)) %>%
         mutate(transformation = "log2(x + 1)"),
  as_tibble(assay(rld)[, 1:2]) %>% mutate(transformation = "rlog"))  # normalization and size factors not needed - automatically detected
...
it gives you the **log2 fold changes** and the **adjusted p-values**, for the last variable in the design formula (the estimates are of the logarithmic fold change log2(treated/untreated). In our case it will AMIL/DMSO and TG/DMSO).

```{r results_DE}
dds_res <- results(fdds)
dds_res
```

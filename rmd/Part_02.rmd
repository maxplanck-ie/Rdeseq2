...
head(log(counts(dds), 2) ,3) 
```

b) We need to stabilize the variance across the mean, for which we can use a **regularized logarithm (RLOG)** transformation. It is slower because it requires fitting a shrinkage term for each sample and each gene. Works well for small datasets (n < 30) and when there is a wide range of sequencing depth across samples (an order of magnitude difference).
...
plot(x,  dnorm(x, mean = mean(x)-10, sd=10),  type = "p", col='green')     # Poisson: mean = variance = lambda
lines(x, dnorm(x, mean = mean(x), sd=10), type = "p", col='blue') # NB:mean < variance; size =  dispersion parameter (shape)
...
```{r export_results}
write.table(dds_res, file="../misc/results.tsv", col.names=T, row.names = T, sep = "\t", quote=F)
```


- use the exported data for downstream analysis: 
  - shiny apps on  MPI-IE website: http://deep5.ie-freiburg.mpg.de:3838/bioinfo_official_apps/landing_page/

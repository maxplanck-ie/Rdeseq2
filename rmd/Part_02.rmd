...
#loading data and metadata (sample sheet) in baseR
...
### SHOULD I KEEP THE 1st COLUMN as 'SAMPLES' OR IS IT OK WITHOUT IT?
# set row names (from tibble to df)
#data <- column_to_rownames(data, var="gene_id") %>% as_tibble()
...
#ggplot(rld_PCA, aes(PC1, PC2, color=condition, shape=celltype)) +
  ...
  # xlab(paste0("PC1: ",percentVar[1])) +
  # ylab(paste0("PC2: ",percentVar[2])) +
  # geom_point(aes(x = rld_PCA[1, 1], y = rld_PCA[1, 2]), shape=3, size=3, color="black") +
  # geom_point(aes(x = rld_PCA[7, 1], y = rld_PCA[7, 2]), shape=1, size=3, color="black")+
  # geom_text(x = 0, y = -1, label="what does the sample mixing mean?", color="black", size=5) +
  # geom_segment(aes(x = -25, y = -3, xend = -60, yend = -8), arrow = arrow(length = unit(0.5, "cm")), color="black", show.legend = FALSE) +   #arrow1
  # geom_segment(aes(x = 25, y = -3, xend = 60, yend = -8), arrow = arrow(length = unit(0.5, "cm")), color="black", show.legend = FALSE)      #arrow2
```
...
#metadata_clean <- metadata[-c(1,7),]
#metadata_clean2 <- metadata %>% filter(!row.names(metadata) %in% c("JJ_CTRL_1", "BM_CTRL_1"))



#rld_PCA_clean <- plotPCA(rlog(dds_clean, blind = TRUE), intgroup=c("condition", "celltype"), returnData=TRUE)
...
# ggplot(rld_PCA_clean, aes(PC1, PC2, color=condition, shape=celltype)) +
#   geom_point(size=3) +
#   scale_shape_manual(values=c(3, 1))+    # change shapes of symbols
#   scale_size_manual(values=c(2, 2))+     # change sizes of symbols
#   xlab(paste0("PC1: ",percentVar[1])) +
...
# would work only for tibbles:
#mean_counts3 <- counts(dds_clean)[,3:5] %>% as_tibble() %>% mutate(rowMeans(.))  
...
#plot(unlist(log10(mean_counts3[,ncol(mean_counts3)]) ), log10(variance_counts), xlim=c(-1, 10), ylim=c(-1, 10))
abline(0,1, col="red")
...
dds <- estimateDispersions(dds_clean)
#plotDispEsts(dds)
```

...
metadata <- read.table("/data/processing3/RdeseqCourse/Data/myeloma/myeloma_meta.tsv", header=TRUE, check.names = TRUE, stringsAsFactor = F)
...
```{r trials}
data <- as.matrix(data)
# zero_index <- sample(length(as.matrix(data)), round(0.9*length(as.matrix(data))), replace=F)
# data[zero_index] <- 1

#keep2 <- rowSums(data) > 1
# modif_data <- data[keep,]
# plot(density(log2(data$BM_CTRL_2)))
# hist(log2(data$BM_CTRL_2))
#rug(log2(data$BM_CTRL_2), col="red")

#data <- matrix(rpois(data[,i], mean(data[,i])))
# data <- matrix(rpois(data[,i], mean(data[,i])))
# 
# plot(density(data))
# hist(data)

# a <- rnorm(data, mean(data), sd=150)
# plot(density(a))
# hist(a)
# 
# a <- rnbinom(data, size=10, mu=mean(data))
# plot(density(a))
# hist(a)
```
...
## 2. Brief overview and (semi-)deep dive into concepts - 30 min
...
> **Task 1.a):** Please reorder factors with 'DMSO' as baseline - 2 min
...
dds$condition
```
> **Task 1.b):** Please reorder factors with 'AMIL' as baseline - 2 min
...
> **Task 2.a):** Please do a logarithmic transformation of the first 4 samples (columns) 

```{r log, echo = FALSE, results = 'hide', message=FALSE}
print('dataset with counts before transformation (raw counts):' )  
head(counts(dds)[,1:4],3)    

print('dataset with counts after logarithmic transformation:' )  
head(log(counts(dds)[,1:4], 2) ,3) 
```
...
> **Task 2.b):** Please do a rlog transformation of the first 4 samples (columns) 

```{r rlog, echo = FALSE, results = 'hide', message=FALSE}
...
```{r logs_plot, echo=FALSE, fig.show="hold", out.width = '50%'}
#op <- par(mfrow=c(1,2))
plot(counts(dds)[,c(3,10)])
plot(assay(rld)[,c(3,10)])
#par(op)
```
...
df1 <- data.frame(rlg_base2=log2(counts(dds)[, 1]+1))
...
th <- data.frame(round(log(counts(dds)+1)))
...
#############################
rld_new3 <- plotPCA(rlog(dds, blind = TRUE), intgroup='celltype', returnData=TRUE)  # 'plotPCA' is a DESeq2 function used for plotting transformed data; can use ggplot code you learned yesterday
...
> **Task 3:** Please plot the mean vs the variance of the genes. Use the following code as guide 

```
#1) calculate the mean and the variance for each gene (the code for the mean is given bellow)
mean_counts <- apply(counts(dds)[,6:8], 1, mean) 

#2) plot using plot() and smoothScatter()
```
...
mean_counts <- apply(counts(dds)[,6:8], 1, mean)        # '1' means that the function 'apply' is applied to rows.
...
#plot(mean_counts, variance_counts, xlim=c(-0.5, 150000), ylim=c(-0.5, 1000000000))
...
> **Task 4.a):** Please print out the sizes of each sample (hint: library size is the sum of all counts)
...
> **Task 4.b):** Please print out the sizes of each sample after normalization
...
Why can you do this in 2 different ways? So you can integrate various methods to compensate for sources of bias within the DESeq2 workflow.
...
> **Poll 3**: for small count means, dispersion $\alpha$ is ....? For large mean counts, dispersion $\alpha$ is ....?
...
```{r Visualise dispersions, out.width = '60%', fig.align="center", message=FALSE, warning=FALSE}
plotDispEsts(dds)
```

Bad examples of dispersion plots:
```{r, echo=FALSE, out.width = '60%', fig.align="center"}
knitr::include_graphics(c("../images/disp_badPlot_1.png", "../images/disp_badPlot_2.png"))
```
...
## 6. Exporting results and further analyses - 10 min
...
#write.table(dds_res, file=wfile, col.names=T, row.names = T, sep = "\t", quote=F)  #tab-sep
write.csv(dds_res, file=wfile, row.names=TRUE)                                      #comma-sep
```
